...
module.exports = async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    ...
    try {
      const r = await fetch(WEBHOOK, {
        ...
        body: JSON.stringify({ campaignId, contactId, name, phone, type, time: time || new Date().toISOString() })
      });
  ...
  if (req.method === 'GET') {
    ...
    try {
      const r = await fetch(`${WEBHOOK}?campaignId=${encodeURIComponent(campaignId)}`);
      const text = await r.text();
      let data = {};
      try { data = JSON.parse(text); } catch(e) {}
